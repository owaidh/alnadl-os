// lib/engage-ledger.js — Phase 5 P5-Inc-3: Experience Ledger + Admin
// Overview + Partner Overview (privacy-scoped).
//
 ...
 * filtered by partnerId for SuperAdmin's own convenience when investigating
 * a specific tenant; never required. */
function getFullLedger({ partnerId, limit } = {}) {
  ...
  if (partnerId) {
    rows = rows.filter(r => JSON.parse(r.context_snapshot_json).partnerId === partnerId);
  }
 ...
 * ONLY, for exactly one tenant. Deliberately queries NONE of: payload text,
 * mechanic name/id, selection_reason, provider/model info, or any other
 * tenant's data — there is no field in this function's SQL that could leak
 * those even by a future accidental change, because they are never
 * SELECTed here at all. Matches §11's explicit Partner Dashboard
 * restriction ("لا يظهر: prompts, model routing, ..., Mechanic Lab
 * internals") by construction, not by a filter applied after the fact. */
function getPartnerOverview(partnerId) {
  const sessions = db.prepare(`
    SELECT es.*, ep.context_snapshot_json FROM engage_session es
    JOIN engage_pass ep ON ep.id = es.pass_id
  `).all().filter(row => JSON.parse(row.context_snapshot_json).partnerId === partnerId);
  ...
  return { offered, completed, byPersonality };
}

module.exports = { getFullLedger, getAdminOverview, getPartnerOverview };
