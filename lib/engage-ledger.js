// lib/engage-ledger.js — Phase 5 P5-Inc-3 (corrective round): Experience
// Ledger + Admin Overview + Partner Overview (SQL-level tenant scoping).
//
 ...
 * WHERE clause (corrective round — this used to pull every tenant's
 * sessions into Node and filter with .filter(), which was not exploitable
 * but was not real tenant scoping at the query level either). Deliberately
 * SELECTs NONE of: payload text, mechanic name/id, selection_reason,
 * provider/model info — there is no column in this function's SQL that
 * could leak those even by a future accidental change, because they are
 * never in the SELECT list here at all. Matches §11's explicit Partner
 * Dashboard restriction ("لا يظهر: prompts, model routing, ..., Mechanic
 * Lab internals") by construction, not by a filter applied after the fact. */
