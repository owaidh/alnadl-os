...
const { cosineSimilarity } = require('./engage-embedding-provider.js');
...
// Cosine similarity 0..1 -- calibrated against REAL computed numbers from
// MockEmbeddingProvider (see docs/PHASE5_GAP_ANALYSIS.md's worked table):
// genuinely unrelated content scored ~0.16-0.22, genuine paraphrases
// (English and Arabic, none hardcoded into any dictionary) scored
// ~0.43-0.60. 0.4 sits cleanly between those two clusters.
const DEFAULT_EMBEDDING_THRESHOLD = 0.4;
const MIN_EMBEDDING_THRESHOLD = 0, MAX_EMBEDDING_THRESHOLD = 1;
...
function resolveNoveltyPolicy(partnerId, propertyId, zoneId) {
  ...
  const thresholdVal = getOverrideValue('zone', zoneId, 'novelty_threshold')
    ...
    ?? DEFAULT_THRESHOLD;
  const embeddingThresholdVal = getOverrideValue('zone', zoneId, 'embedding_threshold')
    ?? getOverrideValue('property', propertyId, 'embedding_threshold')
    ?? getOverrideValue('partner', partnerId, 'embedding_threshold')
  ...
  return {
    ...
    embeddingThreshold: Math.min(MAX_EMBEDDING_THRESHOLD, Math.max(MIN_EMBEDDING_THRESHOLD, embeddingThresholdVal)),
  };
}

function setNoveltyPolicyOverride(scopeType, scopeId, key, value, setBy) {
  if (!['novelty_window_days', 'novelty_threshold', 'embedding_threshold'].includes(key)) throw new Error(`Unknown novelty policy key: ${key}`);
...
function recordExposureAndEvaluation(profileId, mechanicId, candidateContent, momentId, evaluation) {
  ...
  db.prepare(`INSERT INTO exposure_memory (id,profile_id,mechanic_id,content_hash,token_set_json,exposed_at,embedding_vector_json,embedding_model,embedding_model_version) VALUES (?,?,?,?,?,?,?,?,?)`)
    .run(uid('exm'), profileId, mechanicId, contentHash(candidateContent), JSON.stringify(tokens), now,
      evaluation.vector ? JSON.stringify(evaluation.vector) : null, evaluation.model || null, evaluation.modelVersion || null);
 ...
 * remembered exposure). Corrective round: this is honestly renamed to
 * 'semantic_concept_similarity' -- it is a real, useful heuristic
 * (concept-normalization + Jaccard) and measurably better than raw word
 * overlap, but it is NOT vector/embedding-based similarity, and must
 * never claim to be. It remains available as an optional, fast,
 * synchronous pre-filter and as the safety net checkNoveltyEmbedding()
 * degrades to when the real embedding provider fails -- see
 * checkNoveltyEmbedding() below for the genuine embedding-based method
 * that 'semantic_embedding' is now honestly reserved for. */
function checkNoveltyConceptSimilarity(profileId, candidateContent, partnerId, propertyId, zoneId) {
  const { windowDays, threshold } = resolveNoveltyPolicy(partnerId, propertyId, zoneId);
  const windowStart = Date.now() - windowDays * 24 * 3600 * 1000;
  const memories = db.prepare(`SELECT content_hash, token_set_json FROM exposure_memory WHERE profile_id = ? AND exposed_at >= ?`).all(profileId, windowStart);
  ...
  return { isDuplicate: maxSimilarity >= threshold, similarityScore: maxSimilarity, threshold, method: 'semantic_concept_similarity' };
}

/** The genuine embedding-based method. Calls the configured
 * EmbeddingProvider (lib/engage-embedding-provider.js) to vectorize the
 * candidate content, then compares via cosine similarity against every
 * PERSISTED embedding vector for this profile's exposures within the
 * window (embeddings are stored once, at exposure time, in
 * exposure_memory.embedding_vector_json -- comparisons here never
 * re-embed old content). Configurable threshold (embedding_threshold),
 * default 0.4, calibrated against real computed numbers: genuinely
 * unrelated content scored ~0.16-0.22 with the Mock's technique,
 * genuine paraphrases (including ones sharing no words at all beyond
 * common entities) scored ~0.43-0.60 -- see
 * docs/PHASE5_GAP_ANALYSIS.md for the full worked table, including
 * Arabic examples never hardcoded anywhere in this codebase.
 *
 * Graceful degradation (required, not optional): if the embedding
 * provider throws OR exceeds its own timeout, this NEVER propagates an
 * error up to the session -- it falls back to
 * checkNoveltyConceptSimilarity() (itself synchronous and unable to
 * fail) and returns that result with degraded:true so the caller and the
 * Ledger both know honestly that this particular check did not use real
 * vector similarity. */
async function checkNoveltyEmbedding(profileId, candidateContent, partnerId, propertyId, zoneId, embeddingProvider) {
  const { windowDays, embeddingThreshold } = resolveNoveltyPolicy(partnerId, propertyId, zoneId);
  const { EMBEDDING_TIMEOUT_MS } = require('./engage-embedding-provider.js');

  const candidateText = `${candidateContent.title_ar || ''} ${candidateContent.title_en || ''} ${candidateContent.body_ar || ''} ${candidateContent.body_en || ''}`.trim();

  let embedResult;
  try {
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('embedding timeout')), EMBEDDING_TIMEOUT_MS));
    embedResult = await Promise.race([embeddingProvider.embed(candidateText), timeout]);
  } catch (e) {
    const fallback = checkNoveltyConceptSimilarity(profileId, candidateContent, partnerId, propertyId, zoneId);
    return { ...fallback, degraded: true, degradationReason: e.message };
  }

  const windowStart = Date.now() - windowDays * 24 * 3600 * 1000;
  const memories = db.prepare(`SELECT embedding_vector_json FROM exposure_memory WHERE profile_id = ? AND exposed_at >= ? AND embedding_vector_json IS NOT NULL`).all(profileId, windowStart);

  let maxSimilarity = 0;
  for (const mem of memories) {
    const memVector = JSON.parse(mem.embedding_vector_json);
    const sim = cosineSimilarity(embedResult.vector, memVector);
    if (sim > maxSimilarity) maxSimilarity = sim;
  }

  return {
    isDuplicate: maxSimilarity >= embeddingThreshold, similarityScore: maxSimilarity, threshold: embeddingThreshold,
    method: 'semantic_embedding', degraded: false,
    vector: embedResult.vector, model: embedResult.model, modelVersion: embedResult.modelVersion,
  };
}

module.exports = {
  resolveNoveltyPolicy, setNoveltyPolicyOverride, getOrCreateProfile,
  checkNovelty, checkNoveltyConceptSimilarity, checkNoveltyEmbedding, recordExposureAndEvaluation, contentHash, jaccardSimilarity, tokenize,
  ...
  DEFAULT_EMBEDDING_THRESHOLD, MIN_EMBEDDING_THRESHOLD, MAX_EMBEDDING_THRESHOLD,
};
