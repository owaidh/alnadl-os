 ...
 * partner: the same phone number returns the same profile, so exposure
 * memory genuinely accumulates over time for a known customer. An
 * ANONYMOUS pass (no phone captured) gets a FRESH pseudonymous profile
 ...
 * does NOT attempt to. This is the concrete meaning of "Anonymous memory
 * pseudonymous ولا تنشئ customer master تلقائيًا": no persistent identity is
 * fabricated for someone who never gave one, and nothing here ever writes
 * to any Core customer/loyalty table -- Engage Core Isolation holds. */
function getOrCreateProfile(partnerId, identityRef, passId) {
  const now = Date.now();
  const isAnonymous = !identityRef;
  const effectiveIdentityRef = identityRef || `anon:${passId}`;
...
module.exports = {
  ...
  DEFAULT_WINDOW_DAYS, DEFAULT_THRESHOLD,
};
