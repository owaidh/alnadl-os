...
function login(username, password) {
  ...
  // Role Corrective §4 — حالة الشريك تحكم الدخول أيضًا.
  // كشفه اختبار دورة حياة الشريك: المُحلِّل يقول userLogin=false في Closed،
  // لكن مسار الدخول لم يكن يستشيره -- فكان مستخدمو شريك مُغلق يدخلون.
  // يُعامَل كفشل دخول عادي فلا يكشف حالة الشريك التجارية لأحد.
  if (user.partner_scope) {
    const { can } = require('./partner-status.js');
    if (!can(user.partner_scope, 'userLogin')) {
      recordFailedAttempt(username);
      return null;
    }
  }
