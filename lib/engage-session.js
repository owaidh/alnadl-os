...
async function serveNextMoment(sessionAccessToken) {
  ...
  // Corrective round: real production traffic allocation, replacing a
  // query that was hardcoded to category='static_fallback' AND
  // lifecycle_state='promoted' -- which meant a mechanic in Canary state
  // NEVER received any real traffic regardless of its configured
  // canary_percentage, and no Mechanic-Lab-governed mechanic of any other
  // category could ever reach production even once fully Promoted.
  // resolveEligibleMechanicVersion() (lib/engage-mechanic-lab.js) is
  // seeded with profile.id -- the SAME stable Engage-local identity
  // already used throughout Inc-4's exposure memory -- so a customer
  // cannot change their own canary allocation by refreshing or
  // re-entering; the decision depends only on stable identity, never on
  // request timing.
  const { resolveEligibleMechanicVersion } = require('./engage-mechanic-lab.js');
  const mechanicVersion = resolveEligibleMechanicVersion(session.personality, profile.id);
  ...
  if (aiEnabled) {
    ...
    selectionReason = source === 'ai_generated'
      ? `ai_generated: provider_call=${orchResult.providerCallId}, safety=passed, novelty=unique(method=${orchResult.novelty.method}${orchResult.novelty.degraded ? ',degraded=true' : ''},score=${orchResult.novelty.similarityScore.toFixed(2)})`
