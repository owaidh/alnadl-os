...
function serveNextMoment(sessionAccessToken) {
  ...
  const poolIndex = session.ceiling_moments_used % pool.length;
  const content = pool[poolIndex];
  // Honest about what this actually is: static round-robin selection, not
  // AI/novelty-driven — Inc-4/Inc-7 will add real selection logic on top of
  // this same field, not replace the concept of recording a reason at all.
  const selectionReason = `static_round_robin: mechanic=${mechanicVersion.id} pool_index=${poolIndex}/${pool.length}`;
