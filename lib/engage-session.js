// lib/engage-session.js — Phase 5 P5-Inc-2 (corrective round): Session
// creation linked to an Engage Pass, Moment serving from Approved
// Static/Fallback Content, strict Engagement Ceiling enforcement, and
 ...
 * check must cover ANY prior session for this pass. (This exact bug was
 * found and fixed during Inc-2's own initial delivery — the token-based
 * rewrite here does not reopen it: the lookup is still "any session for
 * this pass_id", just reached via token instead of id.) */
 ...
 * throws a 409 if the Ceiling has been reached (including the RESET
 * special case: ceiling_moments_max is ALREADY hard-clamped to 1 at
 * session creation, so this is one uniform check, not a
 * personality-specific branch). An unknown token resolves to nothing —
 * 403, not 404, deliberately: a 404 would let an attacker distinguish
 * "wrong token" from "right token, wrong state" by timing/response shape;
 * a flat 403 for "does not resolve" gives no such signal. */
function serveNextMoment(sessionAccessToken) {
  ...
  db.prepare(`INSERT INTO moment (id,session_id,mechanic_version_id,sequence_index,status,created_at) VALUES (?,?,?,?,?,?)`)
    .run(momentId, session.id, mechanicVersion.id, session.ceiling_moments_used, 'served', now);
