...
const { resolveEngageEnabled } = require('./engage-flags.js');
...
function processOutboxOnce() {
  ...
  for (const row of pending) {
    try {
      ...
      const contractEnabled = !!(sub && sub.status === 'Active' && sub.features && sub.features.engage_enabled === true);
      // Full Feature Flag precedence (Inc-6): Global Safety -> Partner
      // Contract -> Property -> Zone. Contract is still the baseline (a
      // partner must be paying for/subscribed to Engage at all), but now a
      // Global kill switch or a Property/Zone override can further restrict
      // it -- see lib/engage-flags.js for the full precedence resolution.
      const engageOn = resolveEngageEnabled(contractEnabled, order.partner_id, order.property_id, order.zone_id);
      if (!engageOn) {
      ...
      db.prepare(`INSERT INTO engage_pass (id,order_id,identity_ref,context_snapshot_json,status,created_at,expires_at,access_token) VALUES (?,?,?,?,?,?,?,?)`)
        .run(passId, order.id, order.customer_phone || null, JSON.stringify(contextSnapshot), 'active', now, now + PASS_TTL_MS, accessToken);
