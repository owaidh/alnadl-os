...
//
// Before this fix, the production serving path (lib/engage-session.js)
// selected content via a query hardcoded to
// "category='static_fallback' AND lifecycle_state='promoted'" -- meaning
// a mechanic in 'canary' state was NEVER selected regardless of its
// configured canary_percentage (0% real exposure, not 5%), and ANY
// Mechanic-Lab-governed mechanic of a different category could never
// reach real customers even once fully Promoted. Lifecycle governance
// was real, but disconnected from what actually got served -- found and
// confirmed by direct verification before writing any fix.
// ============================================================

/** Deterministic, stable allocation decision: the SAME (allocationSeed,
 * mechanicVersionId) pair ALWAYS hashes to the SAME bucket -- a customer
 * cannot change their canary allocation by refreshing or re-entering,
 * because nothing about the decision depends on when it is computed,
 * only on stable identity. Scoping the hash to the specific
 * mechanicVersionId (not just the seed alone) means multiple simultaneous
 * canary mechanics for different personalities/rollouts never correlate
 * their allocation decisions for the same customer. 10000 buckets (not
 * 100) gives real precision down to 0.01%, so even a 1% canary is
 * accurately representable, not rounded away. */
function isAllocatedToCanary(allocationSeed, mechanicVersionId, canaryPercentage) {
  const crypto = require('crypto');
  const hash = crypto.createHash('md5').update(`${allocationSeed}:${mechanicVersionId}`).digest();
  const bucket = hash.readUInt32BE(0) % 10000;
  const thresholdBucket = Math.round(canaryPercentage * 100); // e.g. 5% -> bucket < 500 out of 10000
  return bucket < thresholdBucket;
}

/** The single function the production serving path calls. Only ever
 * considers lifecycle_state IN ('canary','promoted') -- draft, simulated,
 * held, rejected, and retired are structurally excluded from this query
 * entirely, which is what makes "0% new traffic" true by construction for
 * all of them, not a filter that could be forgotten. 'emerging' is
 * deliberately NOT included here: this increment's explicit scope is
 * enforcing the Canary percentage; emerging's own rollout mechanics are a
 * later concern, and until defined, an emerging mechanic safely continues
 * to receive 0% traffic (same as draft/simulated) rather than an
 * undefined amount. Category is deliberately NOT filtered (the old query's
 * "static_fallback"-only restriction is exactly what made Mechanic-Lab-
 * governed mechanics invisible to real serving) -- any category reaching
 * 'promoted' is now a genuine serving candidate, matching "Promoted
 * mechanics follow normal production eligibility". */
function resolveEligibleMechanicVersion(personality, allocationSeed) {
  const promotedCandidates = db.prepare(`
    SELECT mv.* FROM mechanic_version mv JOIN mechanic m ON m.id = mv.mechanic_id
    WHERE mv.lifecycle_state = 'promoted' AND json_extract(mv.schema_json, '$.personality') = ?
    ORDER BY mv.created_at DESC`).all(personality);
  const baseline = promotedCandidates[0] || null; // most recently promoted wins if several ever coexist

  const canary = db.prepare(`
    SELECT mv.* FROM mechanic_version mv JOIN mechanic m ON m.id = mv.mechanic_id
    WHERE mv.lifecycle_state = 'canary' AND json_extract(mv.schema_json, '$.personality') = ?
    ORDER BY mv.created_at DESC LIMIT 1`).get(personality);

  if (canary && isAllocatedToCanary(allocationSeed, canary.id, canary.canary_percentage)) {
    return canary;
  }
  return baseline; // safe fallback: no eligible canary allocation -> the promoted mechanic, exactly as before this fix
}
...
module.exports = {
  ...
  isAllocatedToCanary, resolveEligibleMechanicVersion,
