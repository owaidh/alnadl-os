...
function createInvite(hostSessionAccessToken, maxParticipants) {
  ...
  // Corrective round: max_participants means the TOTAL room size, the host
  // included -- not "the host, plus up to max_participants invitees on top".
  // The host is therefore inserted as a real engage_participant row
  // (role='host') at creation time, occupying one seat immediately. This is
  // deliberately the ONLY change needed to fix the semantic: joinInvite()'s
  // atomic capacity check (COUNT(*) < max_participants) already counts
  // whatever is in this table, so a host row here makes the existing
  // concurrency-safe logic correctly reserve that seat with zero changes to
  // the check itself -- default 8 now genuinely means host + up to 7
  // invitees, never host + 8 (= 9 total) as before this fix.
  const hostParticipantId = uid('epa');
  db.prepare(`INSERT INTO engage_participant (id,group_room_id,role,display_name,joined_at) VALUES (?,?,?,?,?)`)
    .run(hostParticipantId, roomId, 'host', null, now);

  return { inviteToken, expiresAt: now + DEFAULT_EXPIRY_MS, maxParticipants: max, participantCount: 1 };
}
