...
/** Refund reversal (Q03): inserts NEGATIVE ledger rows proportional to the
 * refunded amount, split across the same outlets and using the SAME model
 * snapshot as the original sale rows for this order — never mutates the
 * original sale rows. This is what makes SUM(eligible_base) over a period
 * correctly exclude refunded revenue without losing the historical record
 * of what was actually sold. */
function recordRefundRevenue(orderId, refundAmount) {
  const saleRows = db.prepare(`SELECT * FROM revenue_ledger WHERE order_id = ? AND type = 'sale'`).all(orderId);
  if (!saleRows.length) return [];
  const totalGross = saleRows.reduce((s, r) => s + r.gross_amount, 0);
  if (totalGross <= 0) return [];
  const now = Date.now();
  const written = [];
  for (const sale of saleRows) {
    const proportion = sale.gross_amount / totalGross;
    const refundShare = refundAmount * proportion;
    const model = JSON.parse(sale.model_snapshot_json);
    // Refund the same way it was earned: recompute amounts for the refunded
    // slice using the SAME model that applied to the original sale.
    const { partnerAmount, alnadlAmount } = computeAmounts(model, refundShare);
    const id = uid('rl');
    db.prepare(`INSERT INTO revenue_ledger (id,order_id,outlet_id,gross_amount,discount_amount,eligible_base,partner_amount,alnadl_amount,model_snapshot_json,created_at,type)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
      .run(id, orderId, sale.outlet_id, round2(-refundShare), 0, round2(-refundShare), round2(-partnerAmount), round2(-alnadlAmount), sale.model_snapshot_json, now, 'refund_adjustment');
    written.push({ id, outletId: sale.outlet_id, refundShare: round2(refundShare) });
  }
  return written;
}

module.exports = { getActiveModel, computeAmounts, recordOrderRevenue, recordRefundRevenue };
