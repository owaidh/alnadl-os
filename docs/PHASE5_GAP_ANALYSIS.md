...
**الحالة: مُسلَّم، بانتظار مراجعتكم النهائية الشاملة لكل Phase 5 قبل إعلان الاكتمال.**

### دورة الحياة: Draft → Simulated → Canary → Emerging/Promoted / Held / Rejected / Retired

**ملاحظة مهمة يستحق ذكرها**: قيد `mechanic_version.lifecycle_state` كان **موجودًا فعليًا منذ Inc-2** (migration 006) ويشمل الحالات الثماني كاملة — لم يكن هناك حاجة لتوسيع القيد، فقط بناء منطق الحوكمة الفعلي فوقه لأول مرة. **الآليات الخمس الثابتة الموجودة مسبقًا (Inc-2) تبقى `promoted` كما هي دون أي تدخّل** — Grandfathering صريح، وليس إعادة حوكمة رجعية.

### الحوكمة الحرجة الخمس — كل واحدة بدليل مباشر

1. **Canary ≤5%** — سقف صلب (`CANARY_HARD_MAX_PERCENTAGE=5`) **غير قابل للتهيئة إطلاقًا حتى من SuperAdmin** — مُختبَر: 10% مرفوضة، 0% مرفوضة، 5% بالضبط مقبولة.
2. **لا ترقية دون عينة دنيا (افتراضي 100)** — قابلة للتهيئة (`SuperAdmin` فقط)، مُختبَرة عند الصفر (رفض) وعند خفض الحد لـ5 (قبول عند استيفائه فعليًا).
3. **لا ترقية مع حادثة أمان مفتوحة** — مُختبَر: حادثة مفتوحة تمنع حتى مع عينة كافية؛ الحل (`SafetyReviewer` فقط) يُزيل المانع فعليًا.
4. **Kill Switch فوري (`SuperAdmin` فقط)** — يعمل من أي حالة غير نهائية، يتجاوز الرسم البياني العادي، **لا يمكن تفويضه** (مُختبَر: `ProductAdmin` يُرفَض بـ403).
5. **AI يقترح آليات لكن لا يعدّل Global Safety Guardrails** — **مُثبَت بنيويًا**: فحص مباشر على نص الكود المصدري لـ`proposeMechanicFromAI()` يُثبت غياب أي كتابة لـ`venue_policy_override` بنطاق Global أو أي إشارة لـ`HARD_CEILING`/Kill Switch — وليس فقط فحص صلاحيات.

### Simulated Sessions — إعادة استخدام حقيقية للمسار الإنتاجي، بلا وصول لعميل حقيقي

`runSimulation()` تستدعي **نفس** `evaluateSafety()` التي تفحص كل لحظة إنتاجية فعلية — **مُختبَرة أنها تلتقط محتوى غير آمن فعليًا** (10/10 حالات فشل لمحتوى حقيقي غير آمن، لا Stub يمرِّر كل شيء دائمًا). النتائج تُخزَّن في `mechanic_simulation_run` فقط — **معزولة بنيويًا** عن `engage_session`/`moment`/`exposure_memory`، فلا مسار كود يمكن لمحاكاة عبره الوصول لعميل حقيقي أو تلويث الـLedger أو تحليلات الشريك.

### خللان حقيقيان اكتُشفا أثناء كتابة اختبار السباق نفسه (وليس بمراجعة الكود لاحقًا)

**الأول**: التصميم الأول لاختبار السباق افترض أن "ترقية" و"إيقاف طارئ" متزامنَين على نفس الآلية يجب أن يتعارضا (فوز واحد فقط). لكن `transitionLifecycle()` **متزامنة بالكامل بلا أي `await` داخلي** — كل استدعاء يقرأ الحالة الحيّة الفعلية دائمًا، فالعمليتان المختلفتان (ترقية ثم إيقاف طارئ يعمل على النتيجة) **تتسلسلان بشكل صحيح** بدل التعارض — وهذا سلوك **صحيح**، وليس خللاً، اكتُشف بالتنفيذ الفعلي لا بالتخمين.

**الثاني، الأهم**: هذا الاكتشاف كشف عن ثغرة حقيقية أدق — تحديث CAS **ذاتي المرجع** (نفس الحالة مصدرًا وهدفًا) كان SQLite يُبلِّغه كـ`changes=1` رغم عدم حدوث أي تغيير فعلي في القيمة! هذا يعني عمليتين متطابقتين متزامنتين (مثال: إيقافان طارئان لنفس الهدف) كانتا ستنجحان **كلتاهما** بصمت. أُصلح بفحص صريح يرفض الانتقال الذاتي (`toState === fromState`) بـ`409 already_in_state` — والآن اختبار السباق الحقيقي (عمليتان متطابقتان متزامنتان فعليًا عبر HTTP) يُثبت: **فوز واحد بالضبط**، والخاسر يحصل على `409` نظيف، وصف تدقيق **واحد فقط** يُكتَب لا اثنان.

### التدقيق الكامل — مُثبَت بكل الحقول المطلوبة

كل انتقال: `reason` (نص حقيقي)، `metrics_snapshot_json` (لقطة **وقت الانتقال نفسه** — عينة=100، حوادث مفتوحة=0 — لا يُعاد حسابها لاحقًا فتنحرف عن الواقع)، `actor` (الهوية الفعلية)، `is_system_decision` (0 للقرار البشري)، `policy_version`. **مُختبَر مباشرة على قاعدة البيانات الفعلية**، ليس افتراضًا.

### شروط القبول

| الشرط | الحالة | الدليل |
|---|---|---|
| دورة الحياة الكاملة الثمانية | ✅ | موجودة منذ Inc-2، حوكمة حقيقية مبنية الآن |
| Canary≤5% صلب | ✅ | 10%/0% مرفوضان، 5% بالضبط مقبولة |
| لا ترقية دون عينة دنيا | ✅ | قابلة للتهيئة، مُختبَرة عند الحدود |
| لا ترقية مع حادثة أمان مفتوحة | ✅ | مُختبَر مع/بدون حل الحادثة |
| Kill Switch فوري، `SuperAdmin` فقط | ✅ | يعمل من أي حالة غير نهائية، `ProductAdmin` مرفوض |
| AI يقترح لكن لا يعدّل Global Safety | ✅ | إثبات بنيوي مباشر على نص الكود |
| تدقيق كامل بكل الحقول | ✅ | `reason`+`snapshot`+`actor`+`system_decision`+`policy_version` |
| اختبار صريح لسباقات Promote/Hold/Kill | ✅ | 3 سيناريوهات، خلل CAS حقيقي اكتُشف ومُصحَّح |
| Simulated تُعيد استخدام المسار الإنتاجي فعليًا | ✅ | نفس `evaluateSafety()`، تُثبِت رفض محتوى غير آمن فعليًا |
| Simulated لا تصل لعميل حقيقي | ✅ | عزل بنيوي — جدول منفصل تمامًا |
| Baseline من 424/424 | ✅ | **484/484 الآن** (424 + 60 اختبارًا جديدًا) |

## P5-Inc-7 — اعتماد نهائي + توضيح تصنيف (بدون إعادة فتح الجولة)

**الحالة: مُعتمَد ✅.** توضيح تصنيف من ALNADL، مُسجَّل هنا حرفيًا للمرجعية المستقبلية:

- **ENG-NOV-001 = "Development/Architecture Done"** — خط أنابيب تشابه Vector حقيقي موجود ويُشغَّل فعليًا. **مقبول**.
- **"Production Semantic Quality Validation" يبقى Pre-Go-Live** — `MockEmbeddingProvider` الحالي يستخدم Character N-gram Feature Hashing، **وليس نموذجًا دلاليًا مُدرَّبًا**. مزوّد Embedding حقيقي مستقبلي **يجب** أن يُثبِت التقاط ترادف على مستوى المعنى (Meaning-Level Paraphrase) — شاملة أمثلة عربية بمفردات/جذور **مختلفة تمامًا**، وليس فقط تشابهًا معجميًا/صرفيًا كما هو حال الـMock الحالي.
- **بندان إضافيان يُحمَلان لـPre-Go-Live**:
  1. مزوّد حقيقي مستقبلي **يجب** أن يدعم إلغاء/إيقاف طلب فعلي (True Cancellation/Abort) عند انتهاء المهلة — وليس فقط التوقف عن انتظار الـPromise (السلوك الحالي في `withTimeout()`)
  2. تعريف سياسة الاحتفاظ/الخصوصية (Retention/Privacy) للمتجهات (Vectors) المُخزَّنة فعليًا في `exposure_memory.embedding_vector_json` — **غير مُعرَّف بعد**، بند مفتوح صراحة

هذه التوضيحات **لا تُعيد فتح Inc-7** ولا تُغيِّر حالته المُعتمَدة — تُسجَّل هنا كسجل دقيق لنطاق العمل المتبقي قبل الإنتاج الفعلي.
