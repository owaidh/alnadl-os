> **Version:** v2.9.6-corrective-closure · **Status:** Rounds 1-3 + الإغلاق التصحيحي · **Last Updated:** 2026-08-17
...
> **تصحيح**: التقرير السابق في هذا المستند ادّعى عدم وجود `PATCH` وعدم وجود `delivery_grouping`. **كلاهما خطأ**، والاختبار الذي "أثبت" `404` كان يستدعي `/api/admin/properties` **بلا `:id`** — أي أنه اختبر مسارًا غير موجود أصلًا ولم يُثبت شيئًا عن `PATCH`. صُحِّح الاختبار والتوثيق ليطابقا الواقع.

| الواقع المُتحقَّق | الحالة |
|---|---|
| `GET /api/admin/properties` | ✅ موجودة (SuperAdmin · PartnerAdmin) |
| **`PATCH /api/admin/properties/:id`** | ✅ **موجودة وتعمل** — تُحرِّر `delivery_grouping` |
| **`delivery_grouping`** | ✅ **موجود في المخطط** (`migrations/002_delivery_grouping.js`) |
| عزل المستأجر على مسار الكتابة | ✅ **مُطبَّق ومُختبَر** — PartnerAdmin يُرفض على عقار شريك آخر |
| `POST` لإنشاء عقار | ❌ **غير موجود** — وهذا وحده المؤجَّل |

**المؤجَّل فعليًا**: إنشاء عقار جديد فقط. تحرير `delivery_grouping` **متاح اليوم** ولا يحتاج بناءً.

## الإغلاق التصحيحي (`v2.9.6`)

| Role | Required Capability | UI | API | Authorization | Tests | Status |
|---|---|---|---|---|---|---|
| **SuperAdmin** | **تفعيل الشريك من الواجهة** | بطاقة الحالة في ملف الشريك | `POST /api/admin/partners/:id/status` (قائمة) | SuperAdmin · سبب إلزامي · تدقيق | 25 | ✅ **Implemented & Verified** |
| **SiteManager** | **استرجاع كامل بحذف المبلغ** | نافذة الاسترجاع | `POST /api/orders/:id/refund` (قائمة) | مسموح له أصلًا · idempotency سليمة | 25 | ✅ |

**فجوة تفعيل الشريك**: بُنيت النقطة واختباراتها في R2 **بلا واجهة**، فكان التفعيل يتطلب استدعاء API يدويًا — معيار FAIL صريح في §16. أُغلقت الآن **بلا نقطة جديدة ولا توسيع صلاحية**: البطاقة تعرض الحالة والقدرات، وتُظهر **فقط** ما في `allowedTransitions` (فالمحجوب يُصرَّح بسببه لا يُعرض كزر يفشل)، وتفرض سببًا، وتُعيد قراءة الحالة من الخادم بعد النجاح.

**تناقض الاسترجاع**: الواجهة تعد بـ«اترك المبلغ فارغًا لاسترجاع كامل» بينما الخادم يرفضه بـ`400 Refund amount must be positive`. الآن **حذف المبلغ = الرصيد المتبقي** المحسوب على الخادم — فلا يمكن للعميل تجاوزه ولا تخمينه. **مُختبَر**: كامل حقيقي · الباقي بعد جزئي · لا تجاوز للمتبقي · idempotency سليمة.
