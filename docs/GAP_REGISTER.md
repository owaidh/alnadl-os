> **Version:** v2.9.6-corrective-closure · **Last Updated:** 2026-08-17
...
| المعرّف | الفجوة | الإغلاق |
|---|---|---|
| **CC-1** | **تفعيل الشريك بلا واجهة** — النقطة والاختبارات موجودة من R2، فكان التفعيل يتطلب API يدويًا (معيار FAIL في §16) | بطاقة حالة في ملف الشريك تعرض `allowedTransitions` فقط · سبب إلزامي · تأكيد · إعادة قراءة من الخادم · 25 اختبارًا |
| **CC-2** | **تناقض الاسترجاع**: الواجهة تعد بالكامل عند ترك المبلغ فارغًا والخادم يرفضه بـ`400` | حذف المبلغ = الرصيد المتبقي المحسوب على الخادم · مُختبَر مع الجزئي وidempotency |
| **CC-3** | **خطأ تدقيق في وثائقي**: ادّعيت عدم وجود `PATCH` و`delivery_grouping` — والاختبار "أثبته" بمسار بلا `:id` | صُحِّح الاختبار والتوثيق ليطابقا الواقع |

## Role & Control Completeness Corrective — Round 3

| المعرّف | الفجوة | الإغلاق |
|---|---|---|
| **RC-7a** | **SiteManager يملك صلاحية استرجاع مالية خلفيًا بلا أي واجهة** — قدرة معطّلة عمليًا | نافذة استرجاع بتأكيد وسجل ومنع إرسال مزدوج · 38 اختبارًا · `v2.9.5` |
| RC-7b | تنبيهات وتجاوزات مهلة بلا سطح | شاشة استثناءات تعرض **ما يحتاج إجراءً** لا بيانات إضافية · `v2.9.5` |
| RC-7c | قسم "قابل للاسترجاع" مبني على طابور يستثني `Delivered` ⇒ **قائمة لا تمتلئ أبدًا** | استُبدل ببحث برقم الطلب — **كشفه التحقق البصري** · `v2.9.5` |

### مؤجَّل بقرار موثَّق

| البند | السبب |
|---|---|
| **إنشاء عقار (`POST`) فقط** | **تصحيح**: `PATCH /api/admin/properties/:id` **موجودة وتعمل**، و`delivery_grouping` **موجود في المخطط** (`migrations/002`)، والعزل مُطبَّق ومُختبَر. الادّعاء السابق كان خاطئًا، والاختبار الذي "أثبته" استدعى مسارًا بلا `:id`. المؤجَّل هو **إنشاء عقار جديد فقط** |
| **Zones/Points لـSiteManager** | الخادم يسمح، لكن رحلته لا تحتاجهما والطابور يعرضهما مع كل تذكرة |

### قائم — P2

بحث شامل · فلاتر محفوظة · إجراءات مجمّعة · شاشات SA01/SA05/SA08
