...
| `ADMIN_BOOTSTRAP_RESET_ID` | ❌ اختياري | معرّف استرجاع **يُستهلك مرة واحدة** · لا حاجة لحذفه بعد النشر |
...
### دخول SuperAdmin في الإنتاج (P0)

**القاعدة:** لا يُقلع الإنتاج وهو بلا **SuperAdmin صالح للدخول**.

«صالح» **مُطابِق حرفيًا** لما يرفضه `lib/auth.js:login()`، شرطًا بشرط: `role = SuperAdmin` · `active = 1` · `password_hash` ليس `NULL` **ولا سلسلة فارغة** · الحالة ليست `pending_activation`.

المطابقة الحرفية مقصودة: تعريف **أوسع** يعني حسابًا يُحتسب صالحًا ولا أحد يستطيع الدخول به (كان `password_hash = ''` يمرّ)، وتعريف **أضيق** يعني استرجاعًا يقع بلا داعٍ على نظام سليم.

| الحالة | السلوك |
|---|---|
| لا SuperAdmin صالح + بيانات البيئة مضبوطة | **يُنشأ أو يُصلَح تلقائيًا** ويُدقَّق — استرجاع إلزامي لا امتياز |
| لا SuperAdmin صالح + بلا بيانات بيئة | **يرفض الإقلاع** (`exit 1`) — خادم يعمل ولا أحد يدخله أسوأ من خادم لا يقلع |
| يوجد SuperAdmin صالح + الاسم في البيئة لا يطابقه | **تحذير صريح في سجل النشر** · لا يُنشأ حساب: صكّ حسابات إدارية من متغيرات البيئة على نظام حيّ سطحُ تصعيد صلاحيات |
| `ADMIN_BOOTSTRAP_RESET_ID=<قيمة فريدة>` | استرجاع صريح **لمرة واحدة**، مُدقَّق ومُسجَّل استهلاكه |

**العطل الذي أُغلق:** كان الشرط «هل توجد مستخدمون؟» لا «هل يوجد SuperAdmin صالح؟». فأي مستخدم سابق في القاعدة — نشر أقدم، حساب شريك، حساب مشغّل — يجعل التهيئة تعود **بصمت**، فيعود الدخول بـ401 بلا سطر واحد يشرح السبب. وتقع هذه الحالة كلما غُيّرت بيانات الاعتماد بعد أول نشر أو رُكّب حجم دائم على قاعدة أُنشئت سابقًا.

**استرجاع الدخول على نشر قائم — بلا خطوة بعد النشر:**

```bash
ADMIN_BOOTSTRAP_USERNAME=<اسم المشرف>
ADMIN_BOOTSTRAP_PASSWORD=<كلمة مرور 12+ محرفًا>
ADMIN_BOOTSTRAP_RESET_ID=<أي قيمة فريدة — uuid أو تاريخ ورقم>
```

**يُستهلك المعرّف مرة واحدة ويُسجَّل في القاعدة.** كل إقلاع لاحق بنفس القيمة **لا يمسّ كلمة المرور إطلاقًا** — فلا حاجة للعودة وحذف المتغيّر، ولا خطر على كلمة مرور غيّرها المشغّل لاحقًا.

**لماذا لا عَلَم منطقي:** `true` دائم بطبيعته. لو بقي في الإعدادات لأعاد تعيين كلمة المرور مع كل إقلاع، فمحا بصمت أي تغيير لاحق لها. ولذلك `ADMIN_BOOTSTRAP_RESET` **مرفوض صراحةً** (`exit 1`) مع الإشارة إلى البديل — لا يُتجاهل بصمت فيظن المشغّل أن الاسترجاع جارٍ.

استرجاع جديد لاحقًا؟ **معرّف جديد.** الاسترجاع ممكن دائمًا، لكن بقرار صريح لا بأثر جانبي لإقلاع.

**العملية ذرّية.** تغيير الحساب + تسجيل الاستهلاك + أثر التدقيق **داخل معاملة واحدة**: إما أن تقع الثلاثة أو لا يقع شيء.

لو تعذّر إتمامها ⇒ **تراجع كامل + رفض إقلاع** (`exit 1`). والسبب أن الحالة النصفية أخطر ما يمكن أن يحدث هنا: كلمة مرور تغيّرت دون أن يُستهلك المعرّف تعني استرجاعًا **يتكرر مع كل إقلاع**، فتُعاد كلمة المرور إلى قيمة نصّية في الإعدادات إلى الأبد — وهو العطل نفسه الذي جاء تصميم «مرة واحدة» ليمنعه. وخادمٌ يعمل بعد استرجاع فاشل أخطر من خادم يتوقف: لا أحد يعرف أتغيّرت كلمة المرور أم لا.

**فكّ القفل بعد محاولات فاشلة:** العدّاد في ذاكرة العملية — **إعادة تشغيل الخدمة تمسحه فورًا**، وهي أسرع مخرج. أو انتظار 15 دقيقة.

**مُثبَت في `tests/superadmin-access.js` (73 تأكيدًا):** قاعدة جديدة · البقاء عبر إعادة التشغيل · قاعدة فيها مستخدم سابق · الاسترجاع الصريح · أربع صور من «مستخدمون بلا SuperAdmin صالح» · رفض الإقلاع بلا بيانات · الاسترجاع لمرة واحدة وعدم تكراره عند إعادة التشغيل · **فشل مُفتعَل لتسجيل الاستهلاك يُثبت التراجع ورفض الإقلاع** · رفض العَلَم المنطقي · **سيناريو النشر القائم كاملًا** · القفل وفكّه · وعدم المساس ببيئة التطوير.
